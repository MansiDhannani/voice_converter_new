...
export default function DialogueStudio() {
  ...
  const renderDialogue = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/dialogue/render-with-voices`, {
        method: "POST",
    ...
    <div style={s.container}>
      ...
      <div style={s.grid3}>
        ...
        <div style={{ position: "relative" }}>
          <VoiceUploadCard
            label="Speaker C"
            speakerLabel={speakerC}
            onSpeakerChange={setSpeakerC}
            file={voiceC}
            onFile={setVoiceC}
            color={SPEAKER_COLOR.C}
            language={langC}
            onLanguage={setLangC}
            disabled={!enableC}
          />
          <label style={s.toggleRow}>
            <input
              type="checkbox"
              checked={enableC}
              onChange={e => setEnableC(e.target.checked)}
              style={{ marginRight: 6 }}
            />
      ...
      {/* Render button */}
      <button
        onClick={renderDialogue}
        disabled={renderStatus === "rendering" || !voiceA || !voiceB || (enableC && !voiceC)}
        style={{
          ...s.renderBtn,
          opacity: (!voiceA || !voiceB || (enableC && !voiceC)) ? 0.5 : 1,
        }}
      >
      ...
      {renderStatus === "rendering" && (
        <div style={s.progressBox}>
          <div style={s.progressTrack}>
            <div style={{ ...s.progressFill, width: "100%", animation: "pulse 1.5s infinite" }} />
          </div>
          <p style={{ ...s.hint, textAlign: "center" }}>
            Synthesizing each line — English via F5-TTS, Korean via XTTS-v2...
          </p>
        </div>
      )}
        ...
        <div style={s.resultBox}>
          <div style={s.row}>
            ...
            <a
              ...
              download={`dialogue_${result.render_id}.wav`}
              style={s.downloadBtn}
            >
              ⬇ Download WAV
            </a>
...
function VoiceUploadCard({
  ...
  return (
    <div style={{
      ...
      {file && (
        <p style={{
          color: file.size > 45 * 1024 * 1024 ? "#f87171" : "#64748b",
          fontSize: 11,
          marginTop: 4,
        }}>
...
const s: Record<string, React.CSSProperties> = {
  ...
  speakerInput: {
    ...
    fontSize: 16, fontWeight: 700, width: 60,
  },
  langSelect: {
    background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)",
    borderRadius: 6, color: "#e0e7ff", fontSize: 12, padding: "3px 6px", outline: "none",
  },
  uploadLabel: {
    display: "block", padding: "10px 14px", borderRadius: 8,
    border: "1px dashed", textAlign: "center" as const,
    color: "#94a3b8", cursor: "pointer", fontSize: 13,
  },
  toggleRow: {
    display: "flex", alignItems: "center", marginTop: 8, cursor: "pointer",
  },
  ...
  textarea: {
    width: "100%", padding: 14, borderRadius: 10, border: "1px solid rgba(255,255,255,0.1)",
    background: "rgba(255,255,255,0.04)", color: "#e0e7ff", fontFamily: "monospace",
  ...
  previewBtn: {
    ...
    background: "transparent", color: "#94a3b8", cursor: "pointer", fontSize: 12, marginLeft: "auto",
  },
  ...
  renderBtn: {
    padding: "12px 0", borderRadius: 10, border: "none", background: "#4f46e5",
